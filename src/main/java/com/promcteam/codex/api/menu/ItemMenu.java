...
import com.promcteam.codex.api.menu.replacements.MenuReplacement;
...
public class ItemMenu implements InventoryHolder {
     ...
     */
    protected ItemMenu(@Nullable Player owner, int size, String title) {
        if (owner != null) this.ownerId = owner.getUniqueId();
        this.size = size;
        this.title = title;
    ...
    protected void click(InventoryClickEvent event) {
        ...
        if (event.getClickedInventory().getHolder().equals(this)) {
            if (clickAction != null) clickAction.onClick(event);
        ...
        } else if (event.getClickedInventory().getHolder().equals(event.getWhoClicked()) && clickSelf != null)
            clickSelf.onClick(event);
    }

    protected void drag(InventoryDragEvent event) {
        if (dragAction != null) dragAction.onDrag(event);
    }
    ...
    public Inventory update() {
        ...
        items.forEach((key, value) -> {
            if (key < size)
                ...
                Debugger.warn(
                        "There are more items then there are slots to put them in. Skipping item in slot " + key);
        });

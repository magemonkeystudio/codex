package com.promcteam.codex.legacy.utils;
...
public final class ComponentUtils {
        ...
        for (int i = 0; i < message.length(); ++i) {
            ...
            if (c == ChatColor.COLOR_CHAR) {
                ...
                if (matcher.region(i, var10).find()) {
                    ...
                    builderComponent.setClickEvent(new ClickEvent(Action.OPEN_URL,
        ...
        while (matcher.find(position)) {
            ...
            switch (formatCode.charAt(0)) {
                ...
                case 's':
                    ...
                    toLegacyText(this_.getWith().get((withIndex != null) ? (Integer.parseInt(withIndex) - 1) : i++),
                            builder);
                default:
                    break;
            }
    ...
    public static int replace(final BaseComponent this_,
                              final String text,
    ...
    static int replace_(BaseComponent component_,
                        TranslatableComponent this_,
                        final String text,
                        final BaseComponent component,
                        int limit) {
        ...
        if (startIndex != -1) {
            ...
            this_.setTranslate(
                    translate.substring(0, startIndex) + toLegacyText(component) + translate.substring(endIndex));
            if (--limit == 0) {
                return 0;
            }
        }
    ...
    static int replace_(final BaseComponent component_,
        ...
        if (startIndex != -1) {
            final int endIndex = startIndex + text.length();
            component_.setClickEvent(new ClickEvent(this_.getAction(),
                    this_.getValue().substring(0, startIndex) + repl + this_.getValue().substring(endIndex)));
            if (--limit == 0) {
                return 0;
            }
        }
    ...
    static int replace_(final BaseComponent component_,
        ...
        if (startIndex != -1) {
            final int endIndex = startIndex + text.length();
            component_.setClickEvent(new ClickEvent(this_.getAction(),
                    this_.getValue().substring(0, startIndex) + toLegacyText(component) + this_.getValue()
                            .substring(endIndex)));
            if (--limit == 0) {
                return 0;
            }

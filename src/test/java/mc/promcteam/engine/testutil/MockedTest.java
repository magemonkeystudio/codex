package mc.promcteam.engine.testutil;
...
import mc.promcteam.engine.testutil.reflection.TestReflectionUtil;
...
public abstract class MockedTest {
    ...
    public void setupServer() {
        ...
        try {
            File itemsJar = new File(server.getPluginsFolder().getAbsolutePath(), "ProMCCore-" + coreVersion + ".jar");
        ...
        reflex.when(() -> Reflex.getClass(startsWith("mc.promcteam"), anyString()))
                .thenCallRealMethod();
        reflex.when(() -> Reflex.getClass(startsWith("mc.promcteam")))
                .thenCallRealMethod();
...
//        ItemUT.setEngine(engine);
        ...
        plugin = MockBukkit.load(NexEngine.class);
    }

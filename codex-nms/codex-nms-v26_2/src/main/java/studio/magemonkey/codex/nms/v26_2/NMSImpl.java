package studio.magemonkey.codex.nms.v26_2;
...
public class NMSImpl implements NMS {
    ...
    public Channel getChannel(@NotNull Player player) {
        ...
        try {
            Field networkManagerField = getField(connection.getClass(), "connection");
            networkManagerField.setAccessible(true);
            Connection networkManager = (Connection) networkManagerField.get(connection);
    ...
    public void sendPacket(@NotNull Player player, @NotNull Object packet) {
        ...
        ((ServerGamePacketListenerImpl) getConnection(player)).send((Packet<?>) packet);
    }

    @Override
    public void openChestAnimation(@NotNull Block chest, boolean open) {
        ServerLevel   world     = ((CraftWorld) chest.getWorld()).getHandle();
        BlockPos      position  = new BlockPos(chest.getX(), chest.getY(), chest.getZ());
        BlockState    blockData = world.getBlockState(position);
        world.blockEvent(position, blockData.getBlock(), 1, open ? 1 : 0);
    }

    @Override
    public void sendAttackPacket(@NotNull Player player, int i) {
        ClientboundAnimatePacket packet = new ClientboundAnimatePacket(((CraftPlayer) player).getHandle(), i);
    ...
    public boolean isTool(@NotNull ItemStack itemStack) {
        ...
        String[] suffixes = {"_AXE", "_HOE", "_SHOVEL", "_PICKAXE", "_SHEARS"};
        return Arrays.stream(suffixes).anyMatch(suffix -> material.name().endsWith(suffix));
    }

    @Override
    public String toJson(@NotNull ItemStack item) {
        return "unsupported in 26.2";
    }

    @Override
    public void setKiller(@NotNull LivingEntity entity, @NotNull Player killer) {
        net.minecraft.world.entity.LivingEntity hit = ((CraftLivingEntity) entity).getHandle();
        hit.setLastHurtByPlayer(((CraftPlayer) killer).getHandle(), 100);
    }
